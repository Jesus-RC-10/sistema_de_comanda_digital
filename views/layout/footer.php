    ...
    function updateURL(sectionId) {
        const newUrl = `${window.location.pathname}?action=admin&seccion=${sectionId}`;
        window.history.pushState({section: sectionId}, '', newUrl);
    }

    function logout() {
        if (confirm('¿Está seguro que desea cerrar sesión?')) {
            window.location.href = '<?php echo BASE_URL; ?>index.php?action=logout';
        }
    }

    function descargarPDF(tipo) {
        window.open('<?php echo BASE_URL; ?>generar_pdf.php?tipo=' + tipo, '_blank');
    }
