...
class Producto {
    ...
    public function crear($data, $imagen = null) {
        ...
        $stmt->bind_param("ssdiis", $data['nombre'], $data['descripcion'], $data['precio'], $data['categoria_id'], $data['stock'], $imagen);
        if ($stmt->execute()) {
    ...
    public function agregarIngredienteReceta($producto_id, $ingrediente_id, $cantidad) {
        $sql = "INSERT INTO recetas_producto (producto_id, ingrediente_id, cantidad) VALUES (?, ?, ?)";
        $stmt = $this->db->prepare($sql);
        $stmt->bind_param("iid", $producto_id, $ingrediente_id, $cantidad);
        return $stmt->execute();
    }
